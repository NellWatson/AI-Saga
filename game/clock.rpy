...
init -1 python:
    ...
    img = ["gui/clock/VintageClockBase450x450.png", "gui/clock/VintageClockHour450x450.png", 
        "gui/clock/VintageClockMinute450x450.png", "gui/clock/VintageClockSecond400x400.png", 
        "gui/clock/DigitalClockBase460x200.png"]
    """
    ...
    class Clock(renpy.Displayable):
        ...
        def __init__(self, ana=True, h=0, m=0, resize=150, sH=True, mil_time=False, **kwargs):
            ...
            self.digital_base_image = im.Scale("gui/clock/DigitalClockBase460x200.png", self.width, self.d_height)
        ...
        def render(self, width, height, st, at):
            ...
            if self.analogue:                
                ...
                tH = Transform(child=self.hour_hand_image, rotate=self.seconds*0.008333, subpixel=True)
            ...
            #This is all the render information for the Digital Clock
                ...
                time = list(Text("{0:02d}".format(item), style="digi_clock", size=ftht) for item in self.get_time())
        ...
        def autoclock(self, st):
            if self.auto_run:
                ...
                if self.old_second != dt:
                    ...
                    if self.seconds % 60 == 0 and store.chance_for_idle_event < _value:
                        if store.current_idle_event:
                            store.current_idle_event = ""
                            store.phone_ringing = False
                            store.anything_in_inbox = False

                        action = renpy.random.sample(["ringing", "email"], k=1)[0]
                        if action == "ringing":
                            store.phone_ringing = True
                        elif action == "email":
                            store.anything_in_inbox = True
